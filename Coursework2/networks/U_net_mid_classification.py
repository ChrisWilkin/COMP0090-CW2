...
class Unet(nn.Module):
    def __init__(self,k=32,in_chns=3,n_segments=2):
        ...
        self.binary_classification = nn.Sequential(BatchNorm2d(4*k), ReLU(inplace=True)
                                           ,Conv2d(4*k, k, kernel_size=3, stride=2, padding=1) # now 16 x 16 x k
                                           ,nn.AvgPool2d((2, 2),stride = 2) # 8 x 8 x k
                                           ,nn.Flatten() #TODO: check if should be flattened from dimension 0 or 1 
                                           ,nn.Linear(k*8*8, 1)) # performing binary classification
        ...
        self.upconv1 = nn.Sequential(nn.Upsample(scale_factor=(2,2), mode='bilinear', align_corners=True), Conv2d(8*k,4*k,kernel_size=3,stride=1,padding=1))
        ...
        self.conv5 = nn.Sequential(BatchNorm2d(8*k),ReLU(inplace=True),Conv2d(8*k,4*k,kernel_size=3,stride=1,padding=1)
                                    ,BatchNorm2d(4*k),ReLU(inplace=True),Conv2d(4*k,4*k,kernel_size=3,stride=1,padding=1))
        self.upconv2 = nn.Sequential(nn.Upsample(scale_factor=(2,2), mode='bilinear', align_corners=True), Conv2d(4*k,2*k,kernel_size=3,stride=1,padding=1))
        ...
        self.conv6 = nn.Sequential(BatchNorm2d(4*k),ReLU(inplace=True),Conv2d(4*k,2*k,kernel_size=3,stride=1,padding=1)
                                    ,BatchNorm2d(2*k),ReLU(inplace=True),Conv2d(2*k,2*k,kernel_size=3,stride=1,padding=1))
        self.upconv3 = nn.Sequential(nn.Upsample(scale_factor=(2,2), mode='bilinear', align_corners=True), Conv2d(2*k,k,kernel_size=3,stride=1,padding=1))
    ...
    def forward(self,x):
        ...
        binary_cls = torch.sigmoid(self.binary_classification(maxpool3))

        return out7, binary_cls