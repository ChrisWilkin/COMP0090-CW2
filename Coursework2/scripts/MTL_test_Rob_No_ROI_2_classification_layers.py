...
body.load_state_dict(torch.load('no_ROI_2_tspL_body.pt', map_location=device))
segment.load_state_dict(torch.load('no_ROI_2_tspL_segmentation.pt', map_location=device))
...
with open('MTL_test_seg_accuracy_no_ROI_2_tspl.csv', 'w') as file:
   file.write('\n'.join(str(i) for i in seg_accuracy ))

with open('IOU_no_ROI_2_tspl.csv', 'w') as file:
    file.write('\n'.join(str(i) for i in IOU_list))
    file.write('\n'+f'{IOU_test}')

# saving accuracy at each epoch to csv file
#with open('IOU_no_ROI.csv', 'w') as file:
 #   file.write('\n'.join(str(i) for i in seg_accuracy ))