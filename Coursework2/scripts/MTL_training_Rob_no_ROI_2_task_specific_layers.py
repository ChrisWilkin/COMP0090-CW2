...
import networks.MTL_classification_mid_branch as MTLmid
...
#Network Components
body = MTLmid.Body(K, IN_CHANNELS, N_SEGS).to(device).double()
segment = MTLmid.Segmentation(K, N_SEGS, body).to(device).double()
...
torch.save(body.state_dict(), f'no_ROI_2_tspL_body.pt')   #Network weights for Body 
torch.save(segment.state_dict(), f'no_ROI_2_tspL_segmentation.pt')    #Netowkr weights for segmentation
