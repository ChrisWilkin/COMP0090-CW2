...
RANDOM_WEIGHTS = True  #Trigger random optimiser steps (randomly choose which task to update each  batch)
ROI_UPDATE = True   #ROI
SEG_UPDATE = True
...
with open('MTL_segment_losses_random_weights.csv', 'w') as file:
    file.write('\n'.join(str(i) for i in seg_losses))

with open('MTL_ROI_losses_random_weights.csv', 'w') as file:   #ROI
    file.write('\n'.join(str(i) for i in roi_losses))   #ROI

with open('MTL_Cls_losses_random_weights.csv', 'w') as file:
...
torch.save(body.state_dict(), f'MTLRandomWeightsBodyk12ep10.pt')   #Network weights for Body 
torch.save(segment.state_dict(), f'MTLRandomWeightsSegk12ep10.pt')    #Netowkr weights for segmentation
torch.save(roi.net.state_dict(), f'MTLRandomWeightsROIk12ep10.pt')   #Network weights for ROI  #ROI
