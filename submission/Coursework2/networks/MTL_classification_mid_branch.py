...
class Segmentation(nn.Module):
    def __init__(self, k, n_segments, body: Body):
        ...
        #final segmenation layer
        self.conv7_2 = Conv2d(k,n_segments,kernel_size=3,stride=1,padding=1)

        #classification specific convolutional layer, followed by linear layer
        self.conv4_2 = nn.Sequential(BatchNorm2d(4*k),ReLU(inplace=True),Conv2d(4*k,4*k,kernel_size=3,stride=1,padding=1)
                                    ,BatchNorm2d(4*k),ReLU(inplace=True),Conv2d(4*k,4*k,kernel_size=3,stride=1,padding=1))
    ...
    def forward(self, x):
        ...
        conv7_1 = self.body(x)
        mid = self.body.mid
        cls1 = self.conv4_2(mid)
        cls = torch.sigmoid(self.cls(cls1))
        #7th and final block
